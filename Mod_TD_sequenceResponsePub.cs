...
namespace ResponsesATSPersonal
{
    ...
    public class Mod_TD_sequenceResponsePub : ResponseTemplateBasePub
    {
        ...
        [Description("Bars back when calculating Mod_TD_sequence")]
        ...
        [Description("UseMod_TD_sequenceFromATSGlobalIndicator")]
        public Boolean UseMod_TD_sequenceFromATSGlobalIndicator { get { return _useMod_TD_sequenceFromATSGlobalIndicator; } set { _useMod_TD_sequenceFromATSGlobalIndicator = value; } }
        ...
        public Mod_TD_sequenceResponsePub() : this(true) { }
        public Mod_TD_sequenceResponsePub(bool prompt)
        {
            _black = !prompt;
            ...
            Indicators = new string[] { "Time", "BuySignalStr", "SellSignalStr", "LastHigh", "LastLow", "LastClose","Money_state"};
        }
        public override void Reset()
        {
            ...
            Hashtable Mod_TD_sequenceInputParamTmp = new Hashtable();
            Mod_TD_sequenceInputParamTmp["lookbackperiod"] = _barsback;
            Mod_TD_sequenceInputParamTmp["minmumsignallength"] = _barsback;
            _Mod_TD_sequenceFromATSGlobalIndicator = new ATSGlobalIndicatorPersonal.Mod_TD_sequence();
            _Mod_TD_sequenceFromATSGlobalIndicator.Param = Mod_TD_sequenceInputParamTmp;
            _Mod_TD_sequenceFromATSGlobalIndicator.Initializatin();
            _isShutDown = false;
            TotalProfit = 0m;
            _pt.Clear();
        }
        ...
        void blt_GotNewBar(string symbol, int interval)
        {
            ...
            // calculate the Mod_TD_sequence using high/low/close prices for so many bars back
            ...
            //Bar _csecondBar = _myBars[_myBars.Count - 4];
            //Bar _thirdBar = _myBars[_myBars.Count - 8];
            //Bar _cforthBar = _myBars[_myBars.Count - 6];
            //Bar _firstBar = _myBars[_myBars.Count - 10];
            //Bar _secondBar = _myBars[_myBars.Count - 9];
            for (int i = 1; i <= 10; i++)
            {
                _barVec[i] = _myBars[_myBars.Count - 1 - i];
            }
            
            if (UseMod_TD_sequenceFromATSGlobalIndicator )
            {
                _Mod_TD_sequenceFromATSGlobalIndicator.UpdateValue(_barVec);
                BuySignal = _Mod_TD_sequenceFromATSGlobalIndicator.GetBuySignal();
                SellSignal = _Mod_TD_sequenceFromATSGlobalIndicator.GetSellSignal();
            }
            //else
            //{
            //    SMA = Calc.Avg(Calc.EndSlice(_blt[symbol].Open(), _barsback));
            //}
            // wait until we have the Mod_TD_sequence+ and the Mod_TD_sequence-
            ...
            if (!_wait[symbol])
            {
                ...
                // if Mod_TD_sequence+ is above Mod_TD_sequence-, buy
                ...
                // otherwise if Mod_TD_sequence+ is above Mod_TD_sequence-, sell
            ...
            sendindicators(new string[] { _time.ToString(),
                                          ...
                                          Money_state.ToString("F5", System.Globalization.CultureInfo.InvariantCulture),
        ...
        Boolean _useMod_TD_sequenceFromATSGlobalIndicator = true;
        int _entrysize = 1;
        ...
        ATSGlobalIndicatorPersonal.Mod_TD_sequence _Mod_TD_sequenceFromATSGlobalIndicator = null;
        bool _black = false;
        Boolean _side = true;
        Decimal _adj = 1;
        Decimal BuySignal = 0;
    ...
    /// <summary>
    /// this is the same as Mod_TD_sequenceResponse, except it runs without prompting user
    /// </summary>
    public class Mod_TD_sequenceResponseAutoPub : Mod_TD_sequenceResponsePub
    {
        public Mod_TD_sequenceResponseAutoPub() : base(false) { }
    }
}
