...
namespace ResponsesATSPersonal
{
    ...
    public class TD_sequenceResponsePub : ResponseTemplateBasePub
    {
        ...
        [Description("Bars back when calculating TD_sequence")]
        ...
        [Description("UseTD_sequenceFromATSGlobalIndicator")]
        public Boolean UseTD_sequenceFromATSGlobalIndicator { get { return _useTD_sequenceFromATSGlobalIndicator; } set { _useTD_sequenceFromATSGlobalIndicator = value; } }
        ...
        public TD_sequenceResponsePub() : this(true) { }
        public TD_sequenceResponsePub(bool prompt)
        {
            _black = !prompt;
        ...
        public override void Reset()
        {
            ...
            Hashtable TD_sequenceInputParamTmp = new Hashtable();
            TD_sequenceInputParamTmp["lookbackperiod"] = _barsback;
            TD_sequenceInputParamTmp["minmumsignallength"] = _barsback;
            _TD_sequenceFromATSGlobalIndicator = new ATSGlobalIndicatorPersonal.TD_sequence();
            _TD_sequenceFromATSGlobalIndicator.Param = TD_sequenceInputParamTmp;
            _TD_sequenceFromATSGlobalIndicator.Initializatin();
            _isShutDown = false;
            TotalProfit = 0m;
            _pt.Clear();
        }
        ...
        void blt_GotNewBar(string symbol, int interval)
        {
            ...
            // calculate the TD_sequence using high/low/close prices for so many bars back
            ...
            if (UseTD_sequenceFromATSGlobalIndicator )
            {
                _TD_sequenceFromATSGlobalIndicator.UpdateValue(_barVec);
                BuySignal = _TD_sequenceFromATSGlobalIndicator.GetBuySignal();
                SellSignal = _TD_sequenceFromATSGlobalIndicator.GetSellSignal();
            }
            //else
            //{
            //    SMA = Calc.Avg(Calc.EndSlice(_blt[symbol].Open(), _barsback));
            //}
            // wait until we have the TD_sequence+ and the TD_sequence-
            ...
            if (!_wait[symbol])
            {
                ...
                // if TD_sequence+ is above TD_sequence-, buy
                if ((BuySignal == 1)&& Money_state == 0)
                {
                    Money_state = 1;
                ...
                // otherwise if TD_sequence+ is above TD_sequence-, sell
                if (SellSignal == 1 && Money_state == 1)
                {
                    Money_state = 0;
        ...
        Boolean _useTD_sequenceFromATSGlobalIndicator = true;
        int _entrysize = 1;
        ...
        ATSGlobalIndicatorPersonal.TD_sequence _TD_sequenceFromATSGlobalIndicator = null;
        bool _black = false;
        Boolean _side = true;
        Decimal _adj = 1;
        Decimal BuySignal = 0;
        Decimal SellSignal = 0;
        Decimal Money_state = 0;           //对应研报里的资金状态，也可用_isFlat
        Bar[] _barVec = new Bar[16];      
    ...
    /// <summary>
    /// this is the same as TD_sequenceResponse, except it runs without prompting user
    /// </summary>
    public class TD_sequenceResponseAutoPub : TD_sequenceResponsePub
    {
        public TD_sequenceResponseAutoPub() : base(false) { }
    }
}
